...
void des_encrypt(char *in, int isize, char *out, uint64_t key)
{
	...
	/* make sure input is 8-byte aligned */
	/* (this library has no built in padding */
	if (isize % 8 != 0)
	{
		kprintf("Input is not 8-byte aligned..\r\n");
		return;
	}
	
	uint nblocks = isize / 8;	/* amount of 64-bit blocks in the input buffer */
	...
	{
		kprintf("Bad parity bits in key...\r\n");
		return;
	}
	...
	for (i = 0; i < 16; i++)
	{
		key_schedule(&a_key[i], &next_key, i);
		if (i != 15)
			a_key[i + 1] = next_key;
	}
